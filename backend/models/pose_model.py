...
class PoseModel(object):
    def __init__(self,landmarks):
        ...
        landmarks = np.arrays(landmarks).reshape((33,3))

        self.left_arm_landmarks = self._normalize_landmarks([landmarks[lmk_idx] for lmk_idx in [11,13,15]])
        self.right_arm_landmarks = self._normalize_landmarks([landmarks[lmk_idx] for lmk_idx in [12,14,16]])
    ...
    def _normalize_landmarks(self,landmarks):
        ...
        """
        #take shoulder position as origin
        shoulder_=landmarks[self.landmark_names.index("shoulder")]
        landmarks-= shoulder_

        #Divide positions by the distance between the wrist and the middle finger
